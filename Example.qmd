...
Now we will investigate the difference between these two parameterisations in a more systematic way. We will generate 100 datasets from the data generating process above with sample sizes $n \in \{50, 1000\}$ and compare the MLEs and posterior medians for $\alpha$ and $\beta$, the causal parameters of interest.
...
comp = function(data, iter = 1e3){
  ...
  fit.ml = fitCausal(data,
                     ...
                     family = c(1, 1, 1))
  MLE_alpha = fit.ml$par[1]
  MLE_beta = fit.ml$par[2]
  ...
  PostMed_alpha = sapply(fit@sim$samples, "[[", "alpha") |> median(na.rm = TRUE)
  PostMed_beta = sapply(fit@sim$samples, "[[", "beta") |> median(na.rm = TRUE)
  ...
  PostMedEta_alpha = sapply(fit_eta@sim$samples, "[[", "alpha") |> median(na.rm = TRUE)
  PostMedEta_beta = sapply(fit_eta@sim$samples, "[[", "beta") |> median(na.rm = TRUE)
  
  # return
  return(list(
    "alpha" = c(MLE_alpha, PostMed_alpha, PostMedEta_alpha),
    "beta" = c(MLE_beta, PostMed_beta, PostMedEta_beta)
    ))
...
storeList = lapply(ns, function(n){
  ...
  store = matrix(NA, nrow = m, ncol = 2*3,
                       dimnames = list(
                         NULL,
                         c("Alpha MLE", "Alpha Post. Median", "Alpha Post. Median (Eta)",
                           "Beta MLE", "Beta Post. Median", "Beta Post. Median (Eta)")
                         ))
  ...
  for (i in 1:m) {
    ...
    res = comp(data, iter = 2e3)
    store[i, 1:3] = res$alpha
    store[i, 4:6] = res$beta
...
# create boxplots
lapply(storeList, function(store){
  
  dat = store[[1]][, 1:3] |> as.data.frame()
  colnames(dat) = c("MLE", "Std. Param.", "Alt. Param.")
  dat = tidyr::pivot_longer(dat, cols = everything())
  
  boxplot(value ~ name, data = dat,
          xlab = "", ylab = "")
