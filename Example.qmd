...
Now we will investigate the difference between these two parameterisations in a more systematic way. We will generate 100 datasets from the data generating process above with $n \in \{50, 1000\}$ and compare the MLEs and posterior medians for $\alpha$ and $\beta$, the causal parameters of interest.
...
comp = function(data, iter = 1e3){
  ...
  # return
  return(list(
    "alpha" = c(MLE_alpha, PostMed_alpha, PostMedEta_beta),
...
for (n in ns) {
  ...
  storeList = c(storeList, list(store))
