...
Now, we try to fit this in a Bayesian fashion. We implement a simple Metropolis algorithm using an uninformative prior, and a (truncated) multivariate normal jump distribution. I manually implemented a log-likelihood function. Since $X$ is binary I will simply take the correlation within each group as a plug-in estimate for the correlation parameter. This plug-in approach is obviously not fully Bayesian (see further below for a fully Bayesian approach).
