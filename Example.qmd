...
par(mfrow = c(6, 2), mar = c(2, 2, 2, 1))

# create boxplots
invisible(
  Map(function(store, n){
    
    pars = rownames(store[[1]])
    lapply(pars, function(par){
      
      dat = sapply(store, function(x) x[par, ]) |> t() |> as.data.frame()
      ...
      boxplot(value ~ name, data = dat, col = 4,
              xlab = "", ylab = "", main = paste0(par, ", n = ", n))
      abline(h = round(mean(dat$value)), col = 2, lty = "dashed")
      
    })
    
    
  }, storeList[c("50", "1000")], ns[1:2])
)
