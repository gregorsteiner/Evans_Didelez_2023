...
colnames(store) <- c("beta", "sigma", "beta_Z", "sigma_Z", "p_X")
store[1, ] <- theta_0
...
# jump distribution parameters
sigma_j <- diag(c(0.001, 0.001, 0.001, 0.001, 0.001, 0.001, 0.001))
...
warm <- 1e3
store <- matrix(NA, nrow = m, ncol = p)
colnames(store) <- c("beta", "sigma", "beta_Z", "sigma_Z", "P_X", "phi_1", "phi_0")
store[1, ] <- theta_0
...
Above, we implicitly used $p_{ZX}^* = p_Z \cdot p_X^*$, where $p_X^*$ is the causal distribution of $X$ (independent of Z) and parameterized this by $\theta_{ZX} = (\mu_Z, \sigma_Z)$. What happens if one uses a different parameterization of $p_{ZX}^*$, e.g. we could use $\eta(\theta_{ZX}) = (\mu_Z / \sigma_Z, 1 / \sigma_Z^2)$.
