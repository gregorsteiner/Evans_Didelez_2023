...
```{r caption = "MLE and posterior medians for the standard parameterisation (Std. Par.) and alternative parameterisation (Alt. Par.)"}
...
# create boxplots
invisible(
  Map(function(store, n){
    
    dat = store[[1]][, 4:6] |> as.data.frame()
    colnames(dat) = c("MLE", "Std. Par.", "Alt. Par.")
    dat = tidyr::pivot_longer(dat, cols = everything())
    dat$name = factor(dat$name, levels = c("MLE", "Std. Par.", "Alt. Par."))
    
    boxplot(value ~ name, data = dat, col = 4,
            xlab = "", ylab = "Beta", main = paste0("n = ", n))
