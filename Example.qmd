...
likl <- function(theta){
  ...
  with(dat, {
    ...
    p_ZX <- dnorm(Z, beta_z, sigma_z, log = TRUE) |> sum() +
      dbinom(X, size = 1, prob = p_X, log = TRUE) |> sum() # not sure about this line
...
prior <- function(theta){
  dnorm(theta[1], 0, 10, log = TRUE) + # beta
    ...
    dbeta(theta[5], 1, 1, log = TRUE) # p_X (the causal probability of treatment assignment)
}

# jump distribution parameters
sigma_j <- diag(rep(0.001, p))

# initial values
theta_0 <- c(0.5, 1.2, 0.5, 0.8, 0.5)

# storage
m <- 5e3
warm <- 1e3
store <- matrix(NA, nrow = m, ncol = p)
colnames(store) <- c("beta", "sigma", "mu_Z", "sigma_Z", "p_X")
store[1, ] <- theta_0
...
```{r}
par(mfrow = c(ceiling(p/2), 2), mar = c(2, 4, 2, 1))
invisible(
  Map(function(i, n){
    hist(store[warm:m, i], main = n, xlab = "",
         freq = FALSE, breaks = 40, col = 2)
  }, 1:p, colnames(store))
)
```
...
prior <- function(theta){
  dnorm(theta[1], 0, 10, log = TRUE) + # beta
    ...
    dnorm(theta[3], 0, 10, log = TRUE) + # mu_Z
    dexp(theta[4], 1/10, log = TRUE) + # sigma_Z
    dbeta(theta[5], 1, 1, log = TRUE) + # p_X
    dunif(theta[6], -1, 1, log = TRUE) + # phi_1
    dunif(theta[7], -1, 1, log = TRUE) # phi_0
}
...
# initial values
theta_0 <- c(0.5, 1.2, 1.5, 0.8, 0.5, 0, 0)

# storage
m <- 5e3
warm <- 1e3
store <- matrix(NA, nrow = m, ncol = p)
colnames(store) <- c("beta", "sigma", "mu_Z", "sigma_Z", "P_X", "phi_1", "phi_0")
store[1, ] <- theta_0
...
for (i in 2:m) {
  ...
  prop <- rmvnorm(1, curr, sigma_j)
  prop[c(2, 4)] <- ifelse(prop[c(2, 4)] <= 0, 1, prop[c(2, 4)])
  prop[5] <- ifelse(prop[5] <= 0 | prop[5] >= 1, 0.5, prop[5])
  prop[6] <- ifelse(prop[6] < -1 | prop[6] > 1, 0, prop[6])
  prop[7] <- ifelse(prop[7] < -1 | prop[7] > 1, 0, prop[7])
...
likl <- function(theta){
  beta <- theta[1]; sigma <- theta[2]
  beta_z <- theta[3] / sqrt(theta[4]); sigma_z <- 1 / sqrt(theta[4])
  p_X = theta[5]
  phi_1 <- theta[6]; phi_0 <- theta[7]
...
prior <- function(theta){
  dnorm(theta[1], 0, 10, log = TRUE) + # beta
    dexp(theta[2], 1/10, log = TRUE) + # sigma
    dnorm(theta[3], 0, 5, log = TRUE) + # eta_1
    dexp(theta[4], 1, log = TRUE) + # eta_2
    dbeta(theta[5], 1, 1, log = TRUE) + # p_X (the causal probability of treatment assignment)
    dunif(theta[6], -1, 1, log = TRUE) + # phi_1
    dunif(theta[7], -1, 1, log = TRUE) # phi_0
}
...
# initial values
theta_0 <- c(0.5, 1.2, 1, 1, 0.5, 0, 0)

# storage
m <- 5e3
warm <- 1e3
store <- matrix(NA, nrow = m, ncol = p)
colnames(store) <- c("beta", "sigma", "eta_1", "eta_2", "P_X", "phi_1", "phi_0")
store[1, ] <- theta_0
...
I was able to implement the model from above in STAN. Here I repeat the analysis from above in STAN, which means multiple chains and better tuning. The only difference is that I sampled $(\alpha_\phi, \beta_\phi)$ instead of $(\phi_1, \phi_0)$ as this was easier to implement in STAN.
...
As above, we have $\eta(\theta_{ZX}) = (\eta_1, \eta_2) = (\mu_Z / \sigma_Z, 1 / \sigma_Z^2)$.
