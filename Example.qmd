...
likl <- function(theta){
  ...
  with(dat, {
    ...
    # estimate plug in estimate for phi by group
    bool <- X == 1
    phi_1 <- cor(Y[bool], Z[bool])
    phi_0 <- cor(Y[!bool], Z[!bool])
...
Here we see that the dependence of $Z$ and $Y$ very much depends on the treatment. Among treated observations the correlation is about 0.5, while it is close to zero among control units.
