...
This notebook works through a simple example using the frugal parameterization proposed by @evans_didelez_2023 . First, we simulate data based on a simple model with an outcome $Y$, a binary treatment $X$, and a confounder $Z$. The setup is
$$ Z \sim N(\mu_Z, \sigma_Z) \\
X | Z = z \sim Ber( \text{expit}(z)) \\
Y | \text{do}(X = x) \sim N(\beta x, \sigma^2).$$

Thus, the frugal parameterization consists of $\theta_{ZX} = (\mu_Z, \sigma_Z)$, $\theta_{Y|X}^* = (\beta, \sigma^2)$, and $\phi_{YZ|X}^*$, the correlation parameter of $Y$ and $Z$ conditional on $X$. The data is simulated with $\beta = 1$, $\sigma^2 = 1$, $\mu_Z = 1$ and $\sigma_Z^2 = 1$.
...
Now, we try to fit this in a Bayesian fashion. We implement a simple Metropolis algorithm using an uninformative prior, and a (truncated) multivariate normal jump distribution. I manually implemented a log-likelihood function, using a Gaussian copula with correlation parameter $\phi = \text{Cor}(Y, Z | X)$. As a first approach, $\phi$ is estimated as a linear function of $X$, but since $X$ is binary I will simply take the mean of each group. This plug-in approach is obviously not fully Bayesian (see further below for a fully Bayesian approach).
...
Here, we try a fully Bayesian approach, that is we do not plug in an estimate for $\phi$, but include it in the parameter vector and also obtain a posterior distribution. However, it is important to keep the dependence on the group, so we sample separately for $\phi_1$ and $\phi_0$, where $\phi_i = \text{Cor}(Y, Z | X = i)$.
...
Again the posterior distribution of $\beta$ is centered around the true value of 1. It seems that the plug-in approach results in a smoother posterior distribution than the fully Bayesian approach.

```{r}
par(mfrow = c(1, 2), mar = c(4, 4, 1, 1))
hist(store[warm:m, 5], breaks = 40, freq = FALSE, 
     main = "", xlab = "Phi_1")
hist(store[warm:m, 6], breaks = 40, freq = FALSE, 
     main = "", xlab = "Phi_0")
```
The posterior distributions for $\phi_1$ and $\phi_0$ look very different.
